...
REM Description: Resets Windows Firewall to default secure policy state.
...
call "%COMMON_LIB%" :Initialize "Firewall_Reset" "1.0.0" "%TOOLKIT_ROOT%"
call "%COMMON_LIB%" :EnsureAdmin "%~f0" %*
if errorlevel 1 exit /b 0

call "%COMMON_LIB%" :Print INFO "Starting Firewall_Reset"

netsh advfirewall reset >> "%LOG_FILE%" 2>&1 & call "%COMMON_LIB%" :RecordResult "Reset firewall policy" %errorlevel%
